...
app.get('/health', async (_req, res) => {
  ...
  const hasLlmKey = Boolean(
    (env.llmProvider === 'gemini' && secrets.gemini) ||
    (env.llmProvider === 'anthropic' && secrets.anthropic) ||
    (env.llmProvider === 'openai' && secrets.openai)
  );
  const aiStatus = hasLlmKey ? 'ok' : 'down';
  const isHealthy = dbStatus === 'ok' && aiStatus === 'ok';
...
for (const route of ROUTES) {
  if (
    route.path === '/health' ||
    route.path === '/stats' ||
    route.path === '/evals/report.json' ||
    route.path.startsWith('/threads') ||
    route.path.startsWith('/memory') ||
...
app.listen(env.port, () => {
  ...
  // Pre-warm GridFS bucket and indexes in background so the first upload incurs 0 cold-start latency
  void (async () => {
    try {
      const b = await (await import('./db.js')).bucket();
      const stream = b.openUploadStream('.warmup');
      stream.end(Buffer.from('warmup'));
      await new Promise<void>((res) => stream.on('finish', () => res()));
      await b.delete(stream.id);
    } catch {
      // ignore
    }
  })();
