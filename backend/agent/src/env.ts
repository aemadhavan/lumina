...
const findProjectRoot = (): string => {
  let cur = process.cwd();
  for (let i = 0; i < 5; i++) {
    if (existsSync(resolve(cur, 'SPEC.md'))) return cur;
...
export const env = {
  ...
  runsDir: resolve(projectRootDir, 'runs')
} as const;
